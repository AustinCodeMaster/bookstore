    ...
    public function databaseinsertion()
    {
        if ($_SERVER["REQUEST_METHOD"] == "POST") {
            ...
            if ($checkResult->num_rows > 0) {
                // User already exists
                header("Location: /bookstore/index.php?error=user_exists");
                exit;
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            try {
                ...
                if ($result->num_rows > 0) {
                    ...
                    echo "<script>alert('Invalid username or Password');window.location.href = '/bookstore/index.php'</script>";
                }
            } catch (Exception $e) {
                echo "Error: " . $e->getMessage();
            }
            header("Location: /bookstore/Global/admin.html");
            exit;
        }
    }
